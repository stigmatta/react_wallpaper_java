        ...
        const fetchOrders = async ()=>{
            try {
                ...
                if (res.ok) {
                    ...
                    const enrichedOrders = await Promise.all(rawData.map(async (order)=>{
                        const enrichedItems = await Promise.all(order.items.map(async (item)=>{
                            ...
                            try {
                                ...
                                if (item.productId) {
                                    const endpoint = item.productType === "WALLPAPER" ? `http://localhost:8080/wallpapers/${item.productId}` : `http://localhost:8080/souvenirs/${item.productId}`;
                                    ...
                                    if (productRes.ok) {
                                        ...
                                        console.warn(`Failed to fetch product details for ${item.productId}: ${productRes.status}`);
                                    }
                                }
                            } catch (e) {
